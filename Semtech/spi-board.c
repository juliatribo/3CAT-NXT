...
void SpiInit( Spi_t *obj, PinNames mosi, PinNames miso, PinNames sclk, PinNames nss )
{
    ...
    if( ( mosi == PB_5 ) || ( mosi == PA_7 ) )
    {
        ...
        if( nss == NC )
        {
            ...
            SpiFormat( obj, SPI_SIZE_8BIT, SPI_POLARITY_LOW, SPI_PHASE_1EDGE, 0 );
        }
        else
        {
            SpiFormat( obj, SPI_SIZE_8BIT, SPI_POLARITY_LOW, SPI_PHASE_1EDGE, 1 );
        }
    }
    else if( mosi == PB_15 )
    {
...
uint16_t SpiInOut( Spi_t *obj, uint16_t outData )
{
    ...
    HAL_SPI_TransmitReceive( &obj->Spi, ( uint8_t * ) &outData, ( uint8_t* ) &rxData, 1, 0xFFFF);
